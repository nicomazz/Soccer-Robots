...
{
 // Serial.println("M3");
    int vel = velocita*molt;//1.5;//molt; // uscite 0..255
    c = velocita;
 //   Serial.print(vel);
    if (vel >= 0)
    {
...
// non più usate. Create per debug iniziale
...
void MuoviAngolo_con_correzione(int angolo, int compass)
{
    ...
    int m3_power = seno[angolo]-compass*3.3f;
    
    m3_power = max(m3_power,-100); m3_power = min(m3_power,100);
    M(coseno[(angolo+330) % 360],coseno[(angolo+30) % 360],m3_power);
}
void M2_continuo()
{
  M2(50);
